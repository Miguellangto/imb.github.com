...
// Function to handle the page transition effect with animation
function handleClick(e) {
    e.preventDefault(); // Prevent immediate page navigation

    const colors = document.querySelectorAll(".color");

    // Start animation for all color elements
    colors.forEach((color) => {
        color.classList.add("active"); // Trigger your transition animation (CSS class)
    });

    // Use the last color to listen for the transition end
    const lastColor = colors[colors.length - 1]; 

    lastColor.addEventListener("animationend", () => {
        // Introduce a 2-second delay before navigating to the next page
        setTimeout(() => {
...
// Attach the click event to all links
